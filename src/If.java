...
//        단순 if문 : 조건식의 결과가 참일 경우 추가로 코드 블럭의 내용을 추가로 실행하는 형태
...
//        else if 문 : 조건식이 여러개가 존재하여 해당 조건식의 결과가 참인 코드 블럭을 실행하는 형태
//        조건식을 여러개 사용하기 때문에 단계별로 조건식을 설정해야 함(문법적으로 문제가 없으나 논리적으로 문제가 발생할 수 있음)
//        마지막에 존재하는 else문은 생략 가능함(else 구문으로 갈 수 없도록 모든 조건을 다 입력했을 경우, else문으로 빠지는 경우)
//
