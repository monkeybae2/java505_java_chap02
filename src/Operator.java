...
//       () 안에 있는 데이터를 먼저 연산
//       연산자의 우선 순위 때문에 왼쪽부터 차례대로 연산을 진행하여 문자열과 숫자(65000)의 연산이 먼저 발생하고, 그 이후에 문자열과 두번째 숫자(2000)과의 연산이 발생하여 '로세티리 바베큐 세트 6500200원' 이라는 형태로 화면에 출력됨
    System.out.println(menu + menu2 + (price + price2) + "원");


    System.out.println("\n----- 증감 연산자 -----\n");

//       전위 연산 혹은 후의 연산이 단독으로 사용될 경우에는 일반 연산자를 통해서 값을 증감시키는 것과 차이가 없음
//       전위 연산 호은 후위 연산을 다른 연산식과 함께 사용할 경우 실행되는 시점이 달라짐
    int op = 10;
    ...
    System.out.println("op-- 실행 시 값 : " + op--);
    System.out.println("현재 op의 값 : " + op);
    System.out.println("--op의 실행 시 값 : " + --op);
    System.out.println("현재 op의 값 : " + op);
